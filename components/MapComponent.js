...
export default class MapComponent extends React.Component {
      ...
      if( ( lat === facility.loc.coordinates[ 1 ] ) &&
        ...
        this.setState(
          ...
          () => {
            this.updateHash(
              {
                lat: facility.loc.coordinates[ 1 ],
                lng: facility.loc.coordinates[ 0 ],
                zoom: (
                  zoom
                  ? _clamp( zoom, 8, 18 )
  ...
  getCurrentPosition = ( errorCallback ) => {
    navigator.geolocation.getCurrentPosition( ( position ) => this.setCurrentPosition( position ), errorCallback );
  }
